...
def test_edit_some_contact_2(app, db, check_ui):
    ...
    with allure.step("Given a data for contact edit"):
    ...
    with allure.step("When I edit %s contact" % contact):
        app.contact.edit_contact_by_id(contact.id, contact_to_edit)
    with allure.step("Then the new list is equal to the old list with the edited contact data"):
